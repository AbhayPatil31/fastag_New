...
import 'package:fast_tag/pages/request_unallocated_complete.dart';
...
List<GetfastagunallocaterequestDatum> fasttagrequestlist = [];
bool nodata = true;

class RequestUnallocatedList extends StatefulWidget {
  @override
  _RequestUnallocatedListState createState() => _RequestUnallocatedListState();
}

class _RequestUnallocatedListState extends State<RequestUnallocatedList> {
  @override
  void initState() {
  ...
  Future<void> Networkcallforfasttagrequestlist(bool showprogress) async {
    try {
      ...
      List<Object?>? list = await NetworkCall().postMethod(
          URLS().get_fastag_unallocate_request_list,
          URLS().get_fastag_unallocate_request_list_url,
          craetejsonString,
          context);
      if (list != null) {
        if (showprogress) {
        ...
        List<Getfastagunallocaterequestlistresponse> response =
            List.from(list!);
                ...
                child: ListView.builder(
                  ...
                  itemBuilder: (BuildContext context, int index) {
                    GetfastagunallocaterequestRequest request =
                        fasttagrequestlist[index].request!;
                    return Card(
  ...
  Widget _buildPendingButtons(
      String requestid, List<GetfastagunallocaterequestDetail> listofdetails) {
    return Row(
              ...
              child: TextButton(
                onPressed: () {
                  Navigator.push(
                    ...
                    MaterialPageRoute(
                        builder: (context) =>
                            UnallocatedRequestCompletePage(requestid)),
                  );
                },
                child: Text(
                  'View',
  ...
  List<Widget> _buildNumberWidgets(
    ...
    for (int i = 0; i < details!.length; i += 3) {
      List<GetfastagunallocaterequestDetail> rowNumbers =
          details!.skip(i).take(3).toList();
      widgets.add(Row(
